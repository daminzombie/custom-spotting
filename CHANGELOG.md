...
- `custom-spotting` is now action-only: model head, training targets, inference, and mAP use `N+1` classes (`background + actions`).
- Checkpoint metadata now records `head_type: "action_only"`; legacy incompatible full checkpoints are not load-compatible with the new head.

## [0.1.0] - 2026-05-07

### Added

- `custom-spotting` package: T-DEED-style temporal model with an action-only head for **broadcast / scene-level** custom labels (`N = 4` foreground actions).
